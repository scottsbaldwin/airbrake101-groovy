...
class Airbrake101 {
    ...
    void demoCase2() {
        ...
        try {
            ...
            // We will customize a notice in order to report
            // some parameters to Airbrake
            Notice n = Airbrake.buildNotice(e);
            n.setParam("depositAmount", depositAmount);
            // We use sendSync instead of reportSync because we
            // customized the notice
            def notice = airbrake.sendSync(n)
            println("An error has been reported, see " + notice.url)
        }
    }
